...
assert.match(source, /stageDeals\.length === 0 && <div data-crm-stage-empty/);
assert.match(source, /window\.setCrmDealStageLocally = handleDealStageChange/);
...
assert.match(app, /function startCrmAnalyticsRealtime\(\)/);
assert.match(app, /table: 'crm_clients'/);
assert.match(app, /table: 'crm_deals'/);
assert.match(app, /if \(viewId === 'crm'\) startCrmAnalyticsRealtime\(\)/);
...
assert.match(app, /window\.setCrmDealStageLocally\?\.\(dealId, 'LOST'\)/);
assert.match(app, /window\.setCrmDealStageLocally\?\.\(dealId, 'WON'\)/);
