...
const start = source.indexOf('function handleTaskPipelineWheel');
const end = source.indexOf("\ndocument.addEventListener('wheel', handleTaskPipelineWheel", start);
assert.ok(start >= 0 && end > start, 'Task pipeline wheel handler was not found');

let textDirection = 'ltr';
const context = { getComputedStyle: () => ({ direction: textDirection }) };
vm.createContext(context);
vm.runInContext(`${source.slice(start, end)}\nglobalThis.testWheelHandler = handleTaskPipelineWheel;`, context);
...
function wheel(board, deltaY, deltaX = 0, deltaMode = 0) {
    ...
    context.testWheelHandler({
        target: { closest: selector => selector === '#tasks-view-board .task-board-wrapper' ? board : null },
        deltaY,
        deltaX,
        deltaMode,
...
console.log('Task pipeline wheel-scroll tests passed.');
